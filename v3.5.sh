...
ask_performance_optimization() {
    ...
    echo -e "${PURPLE}================================================${NC}"
    echo -e "${CYAN}        性能优化选项（新增 v3.5 功能）${NC}"
    echo -e "${PURPLE}================================================${NC}"
    echo ""
...
configure_overclock() {
    ...
    case $OVERCLOCK_LEVEL in
        1) # 保守超频
            ...
            # 根据型号调整参数
            if [[ "$RASPI_MODEL" == *"Raspberry Pi 4"* ]]; then
                sudo tee -a /boot/config.txt > /dev/null << EOF

# 保守超频设置 (v3.5脚本) - 适合树莓派4
over_voltage=2
arm_freq=1750
gpu_freq=550
EOF
...
EOF
                ...
                print_warning "当前型号不支持激进超频，已降级为平衡超频"
                OVERCLOCK_LEVEL=2
                configure_overclock
                return
            fi
...
add_temperature_monitoring() {
    ...
    local monitor_script="$HOME/temp_monitor.sh"
...
while true; do
    ...
    if [ "$temp" != "N/A" ]; then
        if (( $(echo "$temp >= $CRITICAL_TEMP" | bc -l) )); then
            ...
            echo "CRITICAL: $timestamp - 温度过高！当前温度: ${temp}°C" | wall
...
EOF
        ...
        if systemctl is-active temp-monitor.service >/dev/null 2>&1; then
            ...
            add_to_report "温度监控: ⚠ 脚本已创建，服务未启用"
        fi
    else
    ...
    # 创建快速温度检查命令别名
    echo -e "\n# 温度监控别名" >> "$HOME/.bashrc"
    echo "alias temp='vcgencmd measure_temp'" >> "$HOME/.bashrc"
    echo "alias temp-log='tail -20 ~/temp_monitor.log'" >> "$HOME/.bashrc"
    echo "alias temp-monitor='sudo systemctl status temp-monitor.service'" >> "$HOME/.bashrc"
...
reboot_option() {
    ...
    if [ "$AUTO_MODE" = true ]; then
        ...
        read -p "请选择 (1-3，默认1): " -n 1 reboot_choice
        echo
        ...
        case $reboot_choice in
            ...
            2)
                ...
                add_to_report "系统操作：已安排延迟重启"
                ;;
            3)
                print_info "您可以选择稍后手动重启：${GREEN}sudo reboot${NC}"
                add_to_report "系统操作：用户选择手动重启"
                ;;
        esac
