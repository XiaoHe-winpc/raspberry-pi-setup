...
print_header() {
    ...
    if [[ "$USER_CHOICE" == "A" || "$USER_CHOICE" == "a" ]]; then
        ...
        log_message "启用全自动模式"
        echo -e "${YELLOW}注意：全自动模式将自动执行所有优化步骤，无需确认。${NC}"
        sleep 2
    else
        AUTO_MODE=false
...
storage_optimization() {
    ...
    # 全自动模式：自动执行扩展文件系统和TRIM
    if [ "$AUTO_MODE" = true ]; then
        print_info "全自动模式：自动执行存储优化"
        REPLY="y"
    else
        # 交互模式下询问用户
