...
#       4. 新增智能超频与综合性能优化选项
#       5. 新增硬件兼容性检查与温度监控
# 使用方法：sudo bash v3.5.2.sh
# ============================================================================
# 修改记录 v3.5.2
# - 修复 ACTUAL_USER 未定义导致的家目录错误
# - 补全 detect_network_environment 函数，智能网络判断生效
# - 修正 ZRAM 服务名错误（zramswap.service）
# - 超频配置不再重复追加，自动清理旧设置
# - 内存测试改用 sysbench，结果准确
# - 磁盘速度解析更可靠
# - 多处添加 vcgencmd 缺失检查，避免报错
# - 修正 CPU 型号获取方法（适用于树莓派）
# - 恢复备份时仅恢复超频相关行，不覆盖全部 config.txt
# - 电源检测添加空变量保护
# - 温度监控服务添加 WorkingDirectory 并修正用户变量
# - 错误输出保留关键信息，便于调试
# ============================================================================
...
init_environment() {
    ...
    log_message "脚本启动 - 树莓派优化脚本 v3.5.2"
}
...
# ============================================================================
# 【新增】网络环境智能检测（修复全自动模式下国内源/中文支持失效问题）
# ============================================================================
...
check_power_supply() {
    ...
    undervolt=${undervolt:-0}  # 修复：若为空则设为0，避免算术错误
...
chinese_support() {
    ...
    # 安装中文支持（错误输出保留以便调试）
    print_info "安装中文语言包..."
    if sudo apt install -y language-pack-zh-hans language-pack-zh-hant; then
    ...
    print_info "安装中文字体..."
    if sudo apt install -y fonts-wqy-zenhei fonts-wqy-microhei; then
    ...
    print_info "安装中文输入法..."
    if sudo apt install -y ibus ibus-libpinyin ibus-clutter; then
...
configure_sources() {
    ...
    if [ -f /etc/apt/sources.list ]; then
        sudo cp /etc/apt/sources.list /etc/apt/sources.list.backup.$backup_time
    fi
    if [ -f /etc/apt/sources.list.d/raspi.list ]; then
        sudo cp /etc/apt/sources.list.d/raspi.list /etc/apt/sources.list.d/raspi.list.backup.$backup_time
    fi
...
# ============================================================================
# 内存优化 (ZRAM) - 修复：使用正确的服务名 zramswap.service
# ============================================================================
memory_optimization() {
    print_step "内存优化配置"
...
# ============================================================================
# 添加温度监控功能 - 修复：使用 ACTUAL_USER 家目录，添加 WorkingDirectory
# ============================================================================
...
# ============================================================================
# 性能测试 - 修复：内存测试改用sysbench，磁盘速度解析更可靠
# ============================================================================
performance_test() {
    print_step "性能测试"
...
# ============================================================================
# 恢复默认设置功能 - 修复：恢复超频时不覆盖整个config.txt
# ============================================================================
restore_backups() {
    print_step "恢复默认设置选项"
    ...
    case $restore_choice in
        1)
            ...
            ;;
        2)
            # 恢复超频设置 - 仅删除超频相关行，不覆盖整个文件
            if [ -f /boot/config.txt ]; then
                sudo sed -i '/^# 手动超频设置 (v3.5.2脚本)/d' /boot/config.txt
                sudo sed -i '/^over_voltage=/d' /boot/config.txt
                sudo sed -i '/^arm_freq=/d' /boot/config.txt
                sudo sed -i '/^gpu_freq=/d' /boot/config.txt
                sudo sed -i '/^force_turbo=/d' /boot/config.txt
...
main() {
    ...
    echo -e "\n${GREEN}脚本执行完成！感谢使用树莓派优化脚本 v3.5.2！${NC}"
